...
const MKROCoach = () => {
  ...
  // Check if we have enough information to auto-generate a coaching plan
  const hasEnoughInfo = () => {
    return sex && age && height && weight && goal && (days || equipment);
  };

  // Auto-generate initial coaching message when enough info is provided
  useEffect(() => {
    if (hasEnoughInfo() && !hasAutoGenerated && messages.length === 0) {
      setHasAutoGenerated(true);
      const autoMessage = `Hi MKRO! I've filled in my details and I'm ready to get started with my personalized plan. Please generate my complete fitness and nutrition plan based on my information.`;
      
      // Simulate user sending the auto-generated message
      ...
      sendCoachingMessage(autoMessage);
    }
  }, [sex, age, height, weight, goal, days, equipment, hasAutoGenerated, messages.length]);
